...
mt_qeffect = function(
  ...
  # set quantile steps
  if(direction == 'upward'){
    quantiles = seq(1/n_steps,1,length = n_steps)
    } else if(direction == 'downward'){
    quantiles = seq(1,1/n_steps,length = n_steps)      
    } else {
  ...
  if(return_data == TRUE){
    ...
    q = qt(.975,nu[i])
    xlim = c(.5,n_steps + .5)
    ylim = c(floor(min(effect-se*q)),ceiling(max(se*q)))
    plot.new();plot.window(xlim,ylim)
    
    # plot chance
    lines(xlim,c(0,0),col='red',lwd=2)
    ...
    mtext(round(seq(ylim[1],ylim[2],length=10)),at=round(seq(ylim[1],ylim[2],length=10)),side=2,las=1)
