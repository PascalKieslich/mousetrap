...
#' mousetrap plug-ins. From this data.frame, \code{mt_import_mousetrap} creates
#' a mousetrap data object containing the trajectories and additional data for
#' further processing within the mousetrap package. \code{mt_import_mousetrap}
#' returns a list, which includes the trajectory data as an array, and all other
#' data as a data.frame. This data structure can then be passed on to other
...
mt_import_mousetrap <- function(raw_data,
  ...
  if (is.null(mt_id_label)){
    ...
    raw_data[,mt_id] <- paste("id",formatC(ids, width=trunc(log10(nrow(raw_data)))+1, flag="0"),sep="")
  } else {
    if(anyDuplicated(raw_data[,mt_id_label])>0){
      stop(paste("Values in specified mt_id variable -",mt_id_label,"- are not unique."))
    }
  ...
  check_columns <- function(var){
    ...
    if (length(colname)==1){
      ...
      stop(paste("More than one variable in data.frame starts with the label",var,"- please specify a unique label."))
    }else{
      stop(paste("No variable in data.frame starts with the label",var,"- please specify a correct variable."))
    }
  }
  ...
  trajectories <-array(dim=c(nrow(raw_data),3,max_logs),
  ...
  for (i in 1:dim(trajectories)[1]){
    ...
    if (duplicates != "ignore"){
      ...
      if (duplicates %in% c("remove_first","remove_last")){
        ...
        stop("Please specify correct value for duplicates: ignore, remove_first, or remove_last")
      }
...
mt_import_wide <- function(raw_data,
  ...
  if (is.null(pos_ids)){
    message("No pos_ids provided. The following variables were extracting using grep:")
  }
  ...
  for (i in names(mt_variable_labels)){
    ...
    if(!is.null(mt_variable_labels[[i]][["label"]])){
      ...
      if (!is.null(pos_ids)){
        mt_variable_labels[[i]][["cols"]] <- paste(mt_variable_labels[[i]][["label"]],pos_ids,sep=pos_sep)

      # Extract column names using grep otherwise
      } else{
        mt_variable_labels[[i]][["cols"]] <- grep(paste0("^",mt_variable_labels[[i]][["label"]]),colnames(raw_data),value = TRUE)
        message(paste0(i,":"))
  ...
  trajectories  <- array(dim=c(nrow(raw_data),length(mt_variable_labels),max_logs),
  ...
  for (mt_var in mt_variable_labels){
    ...
    # If no timestamp label is provided, create timestamps
      ...
      trajectories[,timestamps,] <- matrix(0:(max_logs-1),nrow = nrow(trajectories),ncol=max_logs,byrow = TRUE)
...
mt_import_long <- function(raw_data,
  ...
  if (is.null(mt_seq_label) | (mt_seq_label %in% colnames(raw_data)==FALSE)){
    message("No mt_seq variable found (that indicates the order of the logs). Automatically created variable based on the timestamps.")
    mt_seq_label <-  "mt_seq"
    raw_data[,"mt_seq"] <- 0
    ...
    for (current_id in unique(raw_data[,mt_id_label])){
      raw_data[raw_data[,mt_id_label]==current_id,mt_seq_label] <- 1:sum(raw_data[,mt_id_label]==current_id)
    }
  }
  ...
  trajectories <- reshape2::melt(raw_data,measure.vars = mt_include,variable.name = "mt_variable")
  custom.formula <- as.formula(paste(mt_id,"mt_variable",mt_seq_label,sep="~"))
  trajectories <- reshape2::acast(trajectories,custom.formula,value.var="value")
  ...
  if (max(table(raw_data[,mt_id]))>1){
    warning("After removing trajectory data, more than one unique row per mt_id remains.")
  }
